...
# Код для создания генеративно-состязательной сети для генерации рукописных цифр, схожих с теми, что находятся в датасете MNIST
...
# Создаёт изображения из случайного шума, пытаясь имитировать настоящие данные
def get_generator(optimizer):
    generator = Sequential()
    generator.add(Dense(256, input_dim=random_dim, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
    generator.add(LeakyReLU(0.2)) # Функция активации
    ...
    generator.add(Dense(784, activation='tanh')) # Функция выходного слоя
    generator.compile(loss='binary_crossentropy', optimizer=optimizer)
    return generator

# Оценивает, насколько реалистично изображение, предоставленное генератором, пытаясь отличить настоящие данные от сгенерированных
def get_discriminator(optimizer):
    discriminator = Sequential()
    discriminator.add(Dense(1024, input_dim=784, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
    discriminator.add(LeakyReLU(0.2)) # Функция активации
    discriminator.add(Dropout(0.3)) # Удаляем ряд нейронов для предотвращения переобучения
    ...
    discriminator.add(Dense(1, activation='sigmoid')) # Возвращает вероятность того, что изображение реально
...
def train(epochs=1, batch_size=128):
    ...
    for e in range(1, epochs+1):
        ...
        for _ in tqdm(range(int(batch_count))):
            ...
            # обучаем дискриминатор (учится отличать реальные данные от фальшивых)
            discriminator.trainable = True
            discriminator.train_on_batch(X, y_dis)

            # обучаем генератор (учится обманывать дискриминатор, делая свои изображения более реалистичными)
            noise = np.random.normal(0, 1, size=[batch_size, random_dim])
